...
2. Place the fixed width files in the RawData folder. They are available at the [official website](https://microdata.gov.in/nada43/index.php/catalog/194) which requires a sign up, or alternatively [here](https://github.com/advaitmoharir/hces_2022/tree/main/03_raw) Some of these files are in `.txt` format while others are in a zip folder. So, extract the `.txt` files from the zip folders before running the scripts.
